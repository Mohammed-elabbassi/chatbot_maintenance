...
def auth_login(req: LoginRequest):
    ...
    # En production : bcrypt.checkpw(req.password, user["mot_de_passe"])
    # Ici : accepter 'admin' comme code universel OU comparer en clair
...
def chat_question(req: QuestionRequest):
    ...
    # Injecter le tenant dans la question si besoin
    question = req.question

    try:
        # Override tenant via le registre
        planner.registry.tenant_aliases["_override"] = req.tenant_db
        result = planner.process_question(question)

        # Forcer le bon tenant si besoin
        if result.get("tenant") in ("jln", "unknown", ""):
            result["tenant"] = req.tenant_db
