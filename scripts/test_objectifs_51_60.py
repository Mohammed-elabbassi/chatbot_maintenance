# test_objectifs_51_60.py
"""
Test des Objectifs 51-60 : Maintenance & Interventions
"""
import sys
...
from app.database.queries_maintenance import (
    get_query, 
    get_objectif_info, 
    list_all_objectifs,
    OBJECTIFS_METADATA
...
def test_all_objectifs():
    """Tester tous les objectifs 51-60"""
    
    db = DatabaseConnection()
    tenant_db = 'v3_tenant_Site_Safi'
    
    print("\n" + "="*80)
    print(" " * 20 + "🧪 TEST DES OBJECTIFS 51-60 : MAINTENANCE")
    print("="*80)
    
    # Vérifier le nombre total d'interventions
    print("\n📋 Statistiques interventions...")
    db.connect(tenant_db)
    query = "SELECT COUNT(*) as total FROM interventions WHERE deleted_at IS NULL"
    result = db.execute_query(query)
    if result:
        total_interventions = result[0]['total']
